...
```bash
podman image inspect rhel-bootc:latest --format "{{.Config.Labels.name}}"
```
...
```bash
podman image pull registry.redhat.io/rhel10/rhel-bootc:latest
```
...
<div style="border: 1px solid #aaa; border-radius: 6px; padding: 12px; background: #020202ff;">
<b>Note:</b><br><br>
You can also omit the content type for the podman inspect command. If the type is not defined, then Podman inspects containers, images, volumes, network, and pods in that order.<br><br>
</div>
...
The following command pushes the `my-httpd` image to the `registry.redhat.io` registry and places it at the `root` of the registry:
